/********************************************************************************
    Modified STL bitset class
    Object size is a multiple of sizeof(char) rather than sizeof(long)
    Copyright (C) 2009
    Adam Martinson
...
#ifndef _ASTD_BITSET_H
#define	_ASTD_BITSET_H
...
#define __LONGT unsigned long long
#define __WORDT unsigned char
#define __WORDS_PER_LONG (sizeof(__LONGT)/sizeof(__WORDT))
#define __BITS_PER_WORD (CHAR_BIT*sizeof(__WORDT))
...
template<size_t _Nw>
struct _Base_bitset {
  typedef __WORDT _WordT;

  _WordT _M_w[_Nw];                // 0 is the least significant word.

  _Base_bitset( void ) { _M_do_reset(); }

  _Base_bitset(__LONGT __val) {
    _M_do_reset();
    for (size_t __i = 0; __i < __WORDS_PER_LONG && __i < _Nw; __i++) {
        _M_w[__i] = static_cast<__WORDT>(__val);
        __val = (__val>>__BITS_PER_WORD);
    }
  }
  ...
  void _M_do_reset() { bzero(_M_w, _Nw * sizeof(_WordT)); }
...
struct _Base_bitset<1UL> {
  ...
  _Base_bitset(__LONGT __val) : _M_w(static_cast<__WORDT>(__val)) {}
  ...
  __LONGT _M_do_to_ulong() const { return _M_w; }
...
template <size_t _Extrabits> struct _Sanitize {
  static void _M_do_sanitize(__WORDT& __val)
    { __val &= ~((~static_cast<__WORDT>(0)) << _Extrabits); }
};

template <> struct _Sanitize<0UL> {
  static void _M_do_sanitize(__WORDT) {}
};
...
public:
  ...
  bitset(__LONGT __val) : _Base_bitset<_Words>(__val) { _M_do_sanitize(); }
  ...
  __LONGT to_ulong() const { return this->_M_do_to_ulong(); }
...
template<size_t _Nw>
__LONGT _Base_bitset<_Nw>::_M_do_to_ulong() const
{
  for (size_t __i = __WORDS_PER_LONG; __i < _Nw; ++__i)
    if (_M_w[__i])
      overflow_error ("bitset");

  __LONGT __val = 0;
  for (size_t __i = 0; __i < __WORDS_PER_LONG && __i < _Nw; __i++)
    __val |= (static_cast<__LONGT>(_M_w[__i])<<(__i*__BITS_PER_WORD));
